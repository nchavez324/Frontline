...
public class Fighter implements Component {
    ...
    public int[][] stocks = {
        ...
        {Weapon.MACHINE_GUN, Weapon.RIFLE, Weapon.SEMI_AUTOMATIC, Weapon.PISTOL, Weapon.MUSKET}, {Weapon.MACHINE_GUN, Weapon.RIFLE, Weapon.SEMI_AUTOMATIC, Weapon.PISTOL, Weapon.MUSKET}
    };
    ...
    public Fighter() {
        ...
        moving = false;
        speed = .15f;
        position = new Vector2((Services.screenWidth - getWidth()) / 2, (600 - getHeight()) / 2);
    ...
    public void draw(Graphics2D g) {
        g.drawImage(legs.animation.sheet,
                (int) position.X - Services.camera.x, (int) position.Y - Services.camera.y,
                (int) position.X + legs.animation.frameWidth - Services.camera.x, (int) position.Y + getHeight() - Services.camera.y,
    ...
    public boolean toHitBounds(int direction) {
        int y2 = 600;
        int y1 = 0;
        int x2 = Services.screenWidth;
        int x1 = 0;
        boolean ans = false;
        if ((direction >= 13 || direction <= 3) && position.Y <= y1
                || direction >= 5 && direction <= 11 && position.Y + legs.getHeight() + upperBody.getHeight() + 20 >= y2
                || direction >= 1 && direction <= 7 && position.X + upperBody.getWidth() >= x2
